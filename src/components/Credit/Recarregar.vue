<template>
  <div class="row">
    <div class="content-header">
      <h1>Recarga | Cliente {{ client.name }} <small>Gerenciamento de Créditos</small></h1>
      <div class="grey-text">
        <small>
      ...
      <div class="card-content">
        <form class="row" autocomplete="off" @submit.prevent="save">
          ...
          <div class="input-field col s6">
            <input type="number" v-model="credit.value" id="value" class="validate" placeholder="Digite o valor da recarga" autocomplete="off" required>
            <label for="value" class="active">Valor</label>
          </div>
          <div class="input-field col s6">
            <input type="text" v-model="credit.validity" id="validity" class="validate" placeholder="Digite a data de validade (yyyy-mm-dd)" autocomplete="off" required>
            <label for="validity" class="active">Validade</label>
          </div>
          <div class="input-field col s6">
          ...
          <div class="input-field col s6">
            <input type="submit" class="waves-effect waves-light btn-large blue-grey darken-4" value="recarregar">
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    save () {
      this.$store.dispatch('recarregar', this.credit).then(() => {
        this.flash('Recarga efetuada com suscesso.', 'success', {
          timeout: 2000
        })
