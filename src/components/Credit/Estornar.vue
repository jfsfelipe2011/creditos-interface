<template>
  <div class="row">
    <div class="content-header">
      <h1>Estornar | Cliente {{ client.name }} <small>Gerenciamento de Créditos</small></h1>
      <div class="grey-text">
        <small>
    ...
    <div class="card col s12">
      <div class="card-content">
        <form class="row" autocomplete="off" @submit.prevent="save">
          ...
          <div class="input-field col s6">
            <input type="submit" class="waves-effect waves-light btn-large blue-grey darken-4" value="estornar">
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    save () {
      this.$store.dispatch('estornar', this.credit).then(() => {
        this.flash('Estorno efetuado com suscesso.', 'success', {
          timeout: 2000
        })
